...
if fillNumber == "0":
  ...
  c = f.LHCFills(0, TDatime(2016,3,29,9,00,00), None, TDatime(2016,3,29,15,00,00))
  #TimeFormat ="%d %b"
  TimeFormat ="%H %M"
else:
...
chain.Draw('bb_phase_mean:daTime-'+begin, 'b1_gain>'+scopeGainCut+' && daTime>'+ begin + '&& daTime<'+end)
...
chain.Draw('b1_phase_mean:daTime-'+begin, 'b1_gain>'+scopeGainCut+' && daTime>'+ begin + '&& daTime<'+end)
gr01= TGraph(gPad.GetPrimitive("Graph"))
chain.Draw('b2_phase_mean:daTime-'+begin, 'b2_gain>'+scopeGainCut+' && daTime>'+ begin + '&& daTime<'+end)
